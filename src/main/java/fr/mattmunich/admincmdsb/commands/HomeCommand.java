...
//		}
		...
		if(label.equalsIgnoreCase("sethome") || label.equalsIgnoreCase("seth"))  {
			...
			if(args.length != 1) {
				player.sendMessage(main.prefix + "§cSintax : /sethome <homeName");
				return true;
			}
...
//					}
			...
			if(args.length != 1) {
				...
				player.sendMessage(main.prefix + "§cSintax : /home <HomeName>");
				return true;
			}
		...
		if(label.equalsIgnoreCase("delhome") || label.equalsIgnoreCase("delh")) {
			...
			if(args.length != 1) {
				player.sendMessage(main.prefix + "§cSintax : /delhome <HomeName>");
				return true;
			}
