...
public class WalkSpeedCommand implements CommandExecutor {
	...
	public WalkSpeedCommand(MonPlugin main) {
		this.main = main;
	}
	...
	public boolean onCommand(CommandSender s, Command cmd, String l, String[] args) {
		...
		if(l.equalsIgnoreCase("walkspeed")) {
			...
			if(args.length > 2 || args.length < 1) {
				p.sendMessage("§cSintax : /walkspeed <nombre/get> [player]");
				return true;
			}
			...
			if(args[0].equalsIgnoreCase("get")) {
				if(args.length == 1) {
					p.sendMessage(main.getPrefix() + "§6La vitesse de marche pour §2" + p.getName() + "§6 est de §2" + p.getWalkSpeed()*10 + "§6 !");
					return true;
				} else {
					...
					p.sendMessage(main.getPrefix() + "§6La vitesse de marche pour §2" + t.getName() + "§6 est de §2" + t.getWalkSpeed()*10 + "§6 !");
					return true;
				}
			} else {
				if(!args[0].matches("-?\\d+")) {
				...
				if(args.length == 2) {
					...
					p.sendMessage(main.getPrefix() + "§6La vitesse de §2" + t.getName() + "§6 pour la marche est maintenant de §2" + n + "§6 !");
					return true;
				} else {
